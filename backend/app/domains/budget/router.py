"""budget 도메인 라우터 — /api/v1/budget/plans (api-spec.md §2 + §5)."""
...
async def upsert_budget_plan(
    ...
    """예산안 upsert (v1.2) — 없으면 201 생성, 있으면 200 갱신."""
    result, created = await service.upsert_budget_plan(db, user, payload)
    if created:
