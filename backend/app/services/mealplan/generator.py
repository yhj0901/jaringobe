...
반환: 끼니 draft 리스트
  {"day":1,"meal_type":"breakfast","name":str,"steps":str|None,
   "ingredients":[{"name":str,"quantity":Decimal,"unit":str}]}
"""
...
_SYSTEM = (
    ...
    "HARD CONSTRAINTS: never include any ingredient the household is allergic to; "
    "keep meals realistic, healthy, and culturally appropriate for the region; "
...
def _prompt(
    household: Household, days: int, meals_per_day: int,
    diet_direction: str, budget_hint: str,
) -> str:
    members = [m.get("age_group") for m in (household.members or [])]
    lines = [
        f"Region: {household.region}",
        f"Household members (age groups): {members}",
        f"Allergies (AVOID strictly): {household.allergies}",
        f"Preferences: {household.preferences}",
        f"Diet direction: {diet_direction}",
...
async def generate_meals(
    household: Household, days: int, meals_per_day: int,
    diet_direction: str, region: str, budget_hint: str = "",
) -> list[dict]:
    llm = get_llm()
    if not llm.enabled:
    ...
    data = await llm.complete_json(
        _SYSTEM, _prompt(household, days, meals_per_day, diet_direction, budget_hint)
    )
